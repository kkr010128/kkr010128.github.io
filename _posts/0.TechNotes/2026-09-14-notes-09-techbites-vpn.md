...
```text
공인 IP:22   → Server 1의 SSH
공인 IP:222  → Server 2의 SSH
공인 IP:2222 → Server 3의 SSH

공인 IP:3389 → Server 1의 RDP
공인 IP:3390 → Server 2의 RDP
공인 IP:5900 → Server 2의 VNC
공인 IP:5959 → Server 3의 VNC
```
...
| 프로토콜 | 특징 |
| --- | --- |
| L2TP/IPsec | L2TP가 데이터를 터널로 전달하고 IPsec이 통신 내용을 보호한다 |
| IKEv2/IPsec | IKEv2로 IPsec 연결을 설정하고 관리한다 |
| OpenVPN | TLS를 이용하며 여러 운영체제에서 사용할 수 있는 오픈 소스 방식이다 |
| WireGuard | 비교적 단순한 구조와 적은 코드로 설계된 VPN 프로토콜이다 |
...
내가 홈서버를 사용하면서 궁금했던 VPN 이야기는 여기까지지만, 앞으로도 다양한 곳에서 더 깊은 기술로 접할 때 빠르게 이해하고 적용할 수 있도록 깊은 내용을 공부해야할 필요를 느꼈다.