...
- LinkedIn 내부 데이터 흐름을 개선하기 위해 개발됨
- 각각의 애플리케이션끼리 연결해서 데이터를 처리하는 것이 아니라 한 곳에 모아 처리할 수 있는 중앙 집중화 방식을 사용하는 것이 가능
- Kafka 내부에 데이터가 저장되는 파티션의 동작은 FIFO
- Queue에 데이터를 저장하는 것이 프로듀서이고, 데이터를 가져가는 것이 컨슈머
- Kafka는 Java로 만들어져 Java에서 선언 가능한 모든 객체를 지원함
- 상용 환경에서는 Kafka는 최소 3대 이상의 서버(브로커)에서 분산 운영되며 프로듀서를 통해 전송받은 데이터는 파일 시스템에 안전하게 기록
- 오픈소스
소스 애플리케이션: 데이터 수집
타겟 애플리케이션: 데이터 저장
...
1. 높은 처리량
2. 확장성
	- Scale-Out, Scale-In의 과정이 쉬움
3. 영속성
	- 데이터를 파일 시스템에 저장하는 것이 가능
4. 고가용

### Data Lake Architecture
- 전통적인 방식은 각 서비스 애플리케이션이 End to End로 데이터를 모아 처리
- 람다 아키텍쳐
	- 속도를 해결하기 위해 카파 아키텍쳐 등장
- 실시간의 경우 스트리밍 데이터 레이크 아키텍쳐

### 설치 및 연습 (Docker Desktop)
- docker 이용
```yaml
version: "3.8"
services:
...
![](../../assets/post/2026-08-11-06-cloud-kafka-cqrs/02.webp)
![](../../assets/post/2026-08-11-06-cloud-kafka-cqrs/04.webp)

### 우분투에 설치 (Virtual Box)
kafka는 Java로 만들어져 있으므로 JDK 먼저 설치, 서버와 클라이언트 함께 진행
```
sudo apt update
...
`
listeners=PLAINTEXT://:9092 # 여기에 의미 추가
advertised.listeners=PLAINTEXT://127.0.0.1:9092 # 사용할 실제 IP, # 여기에 의미 추가

delete.topic.enable=ture # 여기에 의미 추가
auto.create.topics.enable=true # 여기에 의미 추가
`
...
Kafka Heap 메모리 설정(주의할 점)
- EC와 클라우드 인프라의 인스턴스에 설치할 때는 메모리를 확인해야 함
- Kafka와 Zookeeper를 실행하는 데 필요한 메모리는 1.5GB
- 일반적인 인스턴스의 경우 1G 정도의 메모리만 가지고 있으므로 Kafka가 구동되지 않음
- 따라서 더 좋은 인스턴스 사용 / 내부에서 적게 사용하도록 환경 변수에 작업 해야 함

- 전역 환경 변수 설정: `nano ~/.bashrc`
- `export KAFKA_HEAP_OPTS="-Xmx400m -Xms400m`
![](../../assets/post/2026-08-11-06-cloud-kafka-cqrs/06.webp)

### Topic
- Kafka에서 데이터를 구분하기 위해 사용하는 단위
- 1개 이상의 파티션 소유해야 함
- 파티셔에는 프로듀서가 보낸 데이터들이 들어가 저장됨
	- 이 데이터를 레코드라고 함
### Partition
- Kafka의 병렬 처리의 핵심
- 그룹으로 묶인 Consumer들이 레코드를 병렬로 처리할 수 있도록 매칭
- Consumer 개수와 Partition 개수를 늘리면 처리량이 증가함
- 자료 구조에서 큐와 비슷한 구조    
